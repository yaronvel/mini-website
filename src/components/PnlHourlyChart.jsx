...
/**
 * Tight y-range from data: [min, max] with padding; handles flat/constant series.
...
/** @param {Record<string, unknown> & { cx?: number; cy?: number; payload?: { pnl?: number } }} props */
function renderPnlDeltaDot(props) {
  const { cx, cy, payload } = props;
  if (cx == null || cy == null) return null;
  const fill = pnlFill(payload?.pnl);
  return (
    <circle
      cx={cx}
      cy={cy}
...
/** @param {Record<string, unknown> & { cx?: number; cy?: number; payload?: { pnl?: number } }} props */
function renderPnlActiveDot(props) {
  const { cx, cy, payload } = props;
  if (cx == null || cy == null) return null;
  const fill = pnlFill(payload?.pnl);
  return (
...
/**
 * @param {{ hourlySeries: { slot: number; label: string; pnl: number }[] }} props
 */
export function PnlHourlyChart({ hourlySeries }) {
  ...
  return (
    <div className="pnl-hourly-chart-wrap">
      <p className="pnl-hourly-chart-caption">Hourly PnL last 24h</p>
      <ResponsiveContainer width="100%" height={240} minHeight={240}>
        ...
        >
          ...
          <Line
            ...
            stroke="#64748b"
            strokeWidth={2}
            dot={renderPnlDeltaDot}
            activeDot={renderPnlActiveDot}
